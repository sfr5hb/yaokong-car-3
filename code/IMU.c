...
void Angle_seem(float *roll3,float *pitch3,float *yaw3)
{
    ...
    //2、传感器数据滤波
    // 加速度计一阶低通滤波（系数0.8为新数据权重�?.2为历史数据权重）
   ...
   //uef.gz = (0.061f * gz_raw) * 0.0174532925f * 0.8 + 0.2 * bz_prev;
     ...
     // 5. 计算加速度计测量值与预测值的误差（叉积）
      ex = uef.ay * vz - uef.az * vy;  // 横滚轴误�?
      ey = uef.az * vx - uef.ax * vz;  // 俯仰轴误�?
      ez = uef.ax * vy - uef.ay * vx;  // 偏航轴误�?
...
//      exInt = exInt_prev + ex * Ki * DT;  // Ki为积分系数，DT为采样周�?
...
//      ezInt_prev = ezInt;
       ...
       //7.陀螺仪矫正
        ...
        gy_corr = uef.gy + Kp * ey + eyInt; // 比例+积分校正陀螺仪Y�?
        gz_corr = uef.gz + Kp * ez + ezInt; // 比例+积分校正陀螺仪Z�?
